...
class FlashPhotoApp : public mingfx::GraphicsApp {
 public:
  ...
  void ApplyMotionBlurFilter(float radius, ImageEditor::MBlurDir dir);
  ...
  void PublicSaveStateForPossibleUndo();
 ...
 private:
  ...
  FilterThreshold f_thresh;
  FilterChannels f_chann;
  FilterQuantize f_quant;
  FilterSaturate f_sat;

  ConvolutionFilterBlur cf_blur;
  ConvolutionFilterMotionBlur cf_mblur;
  ConvolutionFilterSharpen cf_sharp;
  ConvolutionFilterEdgeDetect cf_edge;
  ...
  // static const std::map<ImageEditor::MBlurDir, std::string> mblur_dir_names_;
};
