...
namespace image_tools {
  ...
  quant_bins_(5) {
    ...
    editor.set_pixel_buffer(current_buffer_);
  }
  ...
  // MOTION BLUR
    ...
    new nanogui::ComboBox(m_blur_params, {
      ImageEditor::MotionBlurDirectionName(ImageEditor::MBLUR_DIR_N_S),
    ...
    b->setFixedSize({150, 30});
    b->setCallback([this] {
      editor.ApplyMotionBlurFilter(mblur_radius_, mblur_dir_);
    });
  ...
  // EDGE DETECT
    nanogui::PopupButton *edge = new nanogui::PopupButton(filters,
      "Edge Detect");
  ...
  // THRESHOLD
    nanogui::PopupButton *thresh = new nanogui::PopupButton(filters,
      "Threshold");
  ...
  void FlashPhotoApp::SaveStateForPossibleUndo() {
    editor.PublicSaveStateForPossibleUndo();
  }

  void FlashPhotoApp::PublicSaveStateForPossibleUndo() {
    editor.PublicSaveStateForPossibleUndo();
  }

  PixelBuffer *FlashPhotoApp::pixel_buffer() {
    return editor.pixel_buffer();
  }

  void FlashPhotoApp::set_pixel_buffer(PixelBuffer *buffer) {
    editor.set_pixel_buffer(buffer);
  }
