...
void PixelBuffer::SaveToFile(const std::string &filename) {
  ...
  for (int i = 0; i < image->Width(); i++) {
    for (int j = 0; j < image->Height(); j++) {
      float red = this->pixel(i, j).red();
      float green = this->pixel(i, j).green();
      float blue = this->pixel(i, j).blue();
      float alpha = this->pixel(i, j).alpha();

      image->SetFloatValue(i, j, 0, red);
      image->SetFloatValue(i, j, 1, green);
      image->SetFloatValue(i, j, 2, blue);
      image->SetFloatValue(i, j, 3, alpha);
    }
  }
...
void PixelBuffer::LoadFromFile(const std::string &filename) {
  ...
  for (int i = 0; i < this->width(); i++) {
    for (int j = 0; j < this->height(); j++) {
      float red = image->FloatValue(i, j, 0);
      float green = image->FloatValue(i, j, 1);
      float blue = image->FloatValue(i, j, 2);
      float alpha = image->FloatValue(i, j, 3);
      ColorData new_color = ColorData(red, green, blue, alpha);
      this->set_pixel(i, j, new_color);
    }
  }
}
